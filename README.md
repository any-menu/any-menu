...
- [x] 允许切换 cm/code miniEditor 引擎
  - 跑一个临时多光标/正则替换的场景
- [ ] 子面板: 实现工具栏子面板 (当前逻辑: 主面板中可以包含任意组合的子面板: 搜索框、多级菜单、编辑框、工具栏(待实现))
- 子面板: 轮盘
...
例如在浏览器环境中，如果你告诉 AI 只需要分享到好友、收藏到浏览器、使用快速剪藏 api 等。AI 无需读取浏览器内容的情况下，浏览器内容并不会输入给 AI。只有你让 AI 去总结、翻译，内容才会给 AI。
此举大大减少了 token 的损耗
