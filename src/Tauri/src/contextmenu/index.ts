...
import { hideWindow } from "../window" // [!code hl] Tauri
import { invoke } from "@tauri-apps/api/core" // [!code hl] Tauri
...
export class ABContextMenu {
  ...
  // 显示该菜单
  private visual_show(x: number, y: number) {
    if (!this.el_container) return
    this.isShow = true
          ...
          // b2. 输出 item.callback 文本到当前光标位置
            ...
            li.addEventListener('click', async () => {
              ...
              if (!activeElement) {
                ...
                await invoke("send", { text: item.callback as string })
              }
