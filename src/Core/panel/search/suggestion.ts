...
// 输入建议
export function createDom_suggestion(el_input: HTMLInputElement, el_input_parent: HTMLElement) {
  // el_suggestion
  const el_suggestion = document.createElement('div'); el_input_parent.appendChild(el_suggestion);
    el_suggestion.classList.add('am-search-suggestion');
    // el_suggestion.setAttribute("id", "autocomplete-list"); // 同一时间应该只会存在一个建议列表
    el_suggestion.style.display = 'none' // 没有匹配项就隐藏
  ...
  el_input.addEventListener('input', (ev) => {
    ...
    search_result = search(el_suggestion, target.value)
  ...
  el_input.addEventListener('keydown', (ev) => {
    ...
    if (ev.key == 'ArrowDown') { // Down 切换选项
      ...
      if (currentFocus > -1) {
        ev.preventDefault()
        if (el_items && search_result.length) el_input.value = search_result[currentFocus].value
      }
    }
  })
}
...
// 执行搜索、并修改输入建议
function search(el_suggestion: HTMLElement, query: string): {key: string, value: string}[] {
